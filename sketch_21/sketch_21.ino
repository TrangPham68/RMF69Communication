...
void loop()
{
  ...
  if (SerialUSB.available() > 0)
  {
    ...
    if ((input == '\r') || (sendlength == 61)) // CR or buffer full
    {
      ...
      SerialUSB.print("sending to node ");
      SerialUSB.print(TONODEID, DEC);
      ...
      if (USEACK)
      {
        if (radio.sendWithRetry(TONODEID, sendbuffer, sendlength))
          SerialUSB.println("ACK received!");
        else
          SerialUSB.println("no ACK received");
      }
  ...
  if (radio.receiveDone()) // Got one!
  {
    ...
    SerialUSB.print(radio.SENDERID, DEC);
    SerialUSB.print(", message [");
